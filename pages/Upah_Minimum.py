...
data = pd.read_csv(
    'disnaker/upah_minimum.csv', sep=',',
    dtype={'kode_kabupaten_kota':'str', 'upah_minimum':'float'}
)
...
#jumlahpenduduk = data.groupby(['nama_provinsi', 'KODE_KK', 'nama_kabupaten_kota', 'tahun'])['upah_minimum'].sum().reset_index()

st.title("Visualisasi Open Data Tenaga Kerja Jawa Barat")
...
if tahunterpilih:
    with st.container(border=True):
        st.subheader(f"Besaran Upah Minimum di Provinsi Jawa Barat, :green[Tahun {tahunterpilih}] (Rupiah)")

    with st.container(border=True):
        kol1, kol2 = st.columns([2,1])
        with kol1:
            fig2 = px.choropleth_mapbox(
                data_frame=datapenduduk[(datapenduduk['tahun'] == tahunterpilih)],
    ...
    with st.container(border=True):
        ...
        with kol3:
            pie_penduduk = px.pie(datapenduduk[(datapenduduk['tahun'] == tahunterpilih)], 
                            values='upah_minimum', names='nama_kabupaten_kota', 
                            color_discrete_sequence=warna_options[pilihwarna])
        ...
        with kol4:
            trimep_penduduk = px.treemap(datapenduduk[(datapenduduk['tahun'] == tahunterpilih)], 
                            values='upah_minimum', path=['nama_provinsi', 'nama_kabupaten_kota'], 
                            color_discrete_sequence=warna_options[pilihwarna])
    ...
    with st.container(border=True):
        perkembangan = px.line(datapenduduk, x='tahun', y='upah_minimum', line_shape='spline',
                               color='nama_kabupaten_kota')
        st.subheader("Perkembangan Upah Minimum di Jawa Barat (Rupiah)")
...
#                 mapbox_style="carto-positron",
#                 hover_name="nama_kabupaten_kota",
#                 hover_data=['tahun', 'jenis_kelamin', 'upah_minimum']
#             )
...
#                         x='tahun', y='upah_minimum', line_shape='spline',
#                         color='nama_kabupaten_kota')
...
with kol1:
    st.link_button("Sumber Data", url="https://opendata.jabarprov.go.id/id/dataset/daftar-upah-minimum-kabupatenkota-di-daerah-provinsi-jawa-barat")
with kol2:
