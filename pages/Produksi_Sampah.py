...
data = pd.read_csv(
    'disrumkim/produksi_sampah.csv', sep=',',
    dtype={'kode_kabupaten_kota':'str', 'produksi_sampah':'float'}
)
...
#jumlahpenduduk = data.groupby(['nama_provinsi', 'KODE_KK', 'nama_kabupaten_kota', 'tahun'])['produksi_sampah'].sum().reset_index()

st.title("Visualisasi Open Data Perumahan dan Permukiman Jawa Barat")
...
if tahunterpilih:
    with st.container(border=True):
        st.subheader(f"Produksi Sampah di Provinsi Jawa Barat, :green[Tahun {tahunterpilih}] (Ton/ Hari)")
    ...
    with st.container(border=True):
        ...
        with kol1:
            fig2 = px.choropleth_mapbox(
                ...
                hover_data=['tahun', 'produksi_sampah']
            )
    ...
    with st.container(border=True):
        ...
        with kol3:
            pie_penduduk = px.pie(datapenduduk[(datapenduduk['tahun'] == tahunterpilih)], 
                            values='produksi_sampah', names='nama_kabupaten_kota', 
                            color_discrete_sequence=warna_options[pilihwarna])
        ...
        with kol4:
            trimep_penduduk = px.treemap(datapenduduk[(datapenduduk['tahun'] == tahunterpilih)], 
                            values='produksi_sampah', path=['nama_provinsi', 'nama_kabupaten_kota'], 
                            color_discrete_sequence=warna_options[pilihwarna])
    ...
    with st.container(border=True):
        perkembangan = px.line(datapenduduk, x='tahun', y='produksi_sampah', line_shape='spline',
                               color='nama_kabupaten_kota')
        st.subheader("Perkembangan Produksi Sampah di Jawa Barat (Ton/ Hari)")
...
with kol1:
    st.link_button("Sumber Data", url="https://opendata.jabarprov.go.id/id/dataset/jumlah-produksi-sampah-berdasarkan-kabupatenkota-di-jawa-barat")
with kol2:
